...
function renderOutstandingReport($conn, $filter_month = 0, $filter_year = 0) {
    $where = ["b.status = 'Partial'"]; // only Partial status
    ...
    echo '<a href="print_outstanding.php?month='.$filter_month.'&year='.$filter_year.'" target="_blank" class="btn btn-secondary ms-2" style="background:#2c3e50; border:none; padding:8px 18px; border-radius:8px;"><i class="fas fa-print me-1"></i> Print Report</a>';
    echo '</div></form>';
    ...
    echo '<input type="text" id="searchTenantOutstanding" class="form-control border-start-0 ps-0" placeholder="Search Tenant...">';
    echo '</div></div>';

    // Table display
    echo '<div class="table-wrapper">';
    echo '<table id="outstandingTable" class="report-table">';
    echo '<thead>';
    echo '<tr>';
    ...
    if (!empty($rows)) {
        ...
        echo '<tr><td colspan="8" class="text-center text-muted py-3">No tenants with partial payments found.</td></tr>';
    }
...
<div class="main-content" style="margin-left:225px; padding:30px; min-height:100vh; background:#f0f4f3;">
    <div class="page-title" style="margin-bottom:24px;">
        <h2 style="font-size:1.6rem;font-weight:700;color:#2c3e50;margin:0;"><i class="fas fa-exclamation-circle" style="color:#5A7D7C;margin-right:8px;"></i> Outstanding Balance Report</h2>
    </div>
    <div style="background:#fff;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,0.08);padding:24px;margin-bottom:20px;">
        <?php renderOutstandingReport($conn, $filter_month, $filter_year); ?>
    </div>
</div>

<!-- Tenant search filter script -->
<script>
