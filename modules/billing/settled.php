...
// Query all billing for current month
$query = "
    ...
    WHERE MONTH(b.due_date) = $currentMonth 
...
while ($row = $res->fetch_assoc()) {
    ...
    $total_amount  = $base_rent + $utility_total + $add_total;

    $payment_amount = floatval($row['payment_amount'] ?? 0);

    // Only include bills that are settled this month
    if ($payment_amount >= $total_amount) {
        $row['total_amount'] = $total_amount;
        $row['credit_balance'] = max($payment_amount - $total_amount, 0);
        $settledTenants[] = $row;
    }
}
?>
...
<div class="d-flex justify-content-between align-items-center mt-0">
    <h3>Settled Tenants (<?php echo date("F Y"); ?>)</h3>
</div>
...
<p class="total-settled">Total Settled Tenants: <?php echo count($settledTenants); ?></p>
